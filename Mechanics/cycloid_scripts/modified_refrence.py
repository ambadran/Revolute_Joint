...
for pin_angle in np.linspace(0,360,num=number_of_pins+1):
    ...
    x_coord = pin_circle_radius*cos(pin_angle)+rolling_circle_radius - contraction
    y_coord = pin_circle_radius*sin(pin_angle)

    pincircle = plt.Circle((x_coord, y_coord), pin_radius)

    pin_coord_lists.append((x_coord, y_coord))
...
def circle_coords_to_dxf2(version: str, coords_list: list[list[list[float, float]]], file_name: str) -> None:
    '''
    converts verts to dxf file
...
circle_coords_to_dxf2('R2010', pin_coord_lists, '../pins.dxf')
